...
public class Room {
    ...
    public Point[][] space; // Space dalam ruangan untuk meletakkan furniture dengan bentuk Point (x,y) (x:
                            // Furniture Number, y: Furniture dengan tipe x ke berapa di ruangan)
    ...
    // Untuk mengecek ada furniture lain di posisi yang akan diletakkan furniture
    // atau mengecek apakah furniture yang akan diletakkan melewati border atau
    // tidak
    public void checkFilled(Point placement, int rotation, Furniture furniture) throws OverlapingRoomObjectException {
        ...
        {
            x = furniture.getSize().getWidth();
            y = furniture.getSize().getHeight();
    ...
    public void placeFurniture(Point placement, int rotation, Furniture furniture)
            throws OverlapingRoomObjectException {
        try {
        ...
        furnitureCount[furniture.getId() - 1]++; // Penambahan jumlah furniture untuk memberikan kode pada furniture
                                                 // yang diletakkan
    ...
    public void removeFurniture(Point placement) {
        if (space[placement.getY()][placement.getX()] != null) // Mengecek placement yang dimasukkan ada furniture atau
                                                               // tidak
        {
            ...
            for (int i = 0; i < 6; i++) // Looping untuk menghilangkan furniture dari space
            {
                for (int j = 0; j < 6; j++) {
                    if (space[i][j] != null) {
                        ...
                        {
                            space[i][j] = null;
                        } else if (space[i][j].getX() == removedfurnitureId.getX()
                                && space[i][j].getY() > removedfurnitureId.getY()) // Melakukan pengurangan terhadap
                                                                                   // furniture lain yang bertipe sama
                        {
    ...
    // Mengembalikan point furniture pada ruangan dengan furnitureX merupakan
    // furniture ke x di ruangan tersebut
    ...
    // Mencetak ruangan beserta sim dan furniture yang ada di dalamnya ( x,y
    // (simNumber))
